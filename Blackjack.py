...
Config = {'Number of players': 0, 'Money': 100, 'Minimum_bet': 5, 'show': True, 'game': 0, 'deck_size': 0,
          'max_deck': 0}
used_cards = {}
Croupier_names = ['Max', 'Rachel', 'Joey', 'Monica', 'Rob', 'Phoebe', 'John', 'Ian', 'Becca', 'Sam', 'Dean']


def sum_card_values(card_values):
...
def draw_cards(quantity, player, deck, deck_value):
...
def config():
    ...
    while True:
        ...
        if configuration not in ['d', 'c']:
            ...
            while True:
                print('How much should minimum bet be?')
                min_bet = input("#: ")
                if min_bet.isdigit() and 0 < int(min_bet) < 100:
                    Config['Minimum_bet'] = int(min_bet)
                    break
            while True:
                print('How much funds should each player have?')
                funds = input("#: ")
                if funds.isdigit() and Config['Minimum_bet'] * 2 < int(funds):
                    Config['Money'] = int(funds)
                    break
            while True:
                print('Croupier should show you one card?')
                show = input("y/n: ").lower()
                if show == 'y':
                    Config['show'] = True
                    break
                if show == 'n':
...
def active_players():
    ...
    if np.isdigit() and 1 <= int(np):
        ...
        Config['max_deck'] = Config['Number of players'] // 2
        if Config['max_deck'] == 0:
            Config['max_deck'] = 1
...
def first_round():
    ...
    for x in Players:
        if Players[x]['Active']:
            draw_cards(2, Players[x], 'Deck1', 'c_values1')
    draw_cards(2, Croupier, 'Deck1', 'c_values1')
    for x in Players:
        if Players[x]['Active']:
    ...
    for x in Players:
        if Players[x]['Active'] and Players[x]['Play']:  # Check for Blackjack first on both sides
            ...
            if (cards[0] == 'Ace' and Deck.get(cards[1]) == 10) or (Deck.get(cards[0]) == 10 and cards[1] == 'Ace'):
                ...
                pause()
                game_resolution()  # if Croupier has blackjack, no need to look more
    for x in Players:
    ...
    if player['Money'] >= (player['Bet'] * 2):
        ...
        while True:
            ...
            if yes_no == 'y':
                ...
                draw_cards(1, player, 'Deck1', 'c_values1')
        ...
        if val > 21:
            ...
            while True:
                ...
                if choice == 's':
                    ...
                    draw_cards(1, player, deck, deck_value)
                    break
...
def double_ace(player, deck, value, score):
    draw_cards(1, player, deck, value)
    print_deck(player, deck)
    val = sum_card_values(player[deck])
    player[score] = val + 10
...
def croupier():
    print("\nIt's my turn.")
    while Croupier['Play']:
        val = sum_card_values(Croupier['c_values1'])
